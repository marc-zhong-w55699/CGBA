...
# ============================================================================
# ImageNet M2D-explore (λ=0.2,0.2,0.6)  v13 = v12.1 base (halving 0.25°), NO DCT
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 verbose_control='Yes',
                 dct_ratio=1.0/8,   # ★ v13: unused (kept for API), full-dim randn used instead
        ...
        self._H, self._W = H, W
        # ★ v13.1: DCT kept ONLY for find_random_adversarial (initial boundary).
        # Main loop (manifold_search_2d) uses full-dim torch.randn for u diversity.
    ...
    def _low_freq_dct(self, shape):
        """★ v13.1: DCT low-freq random — used only in find_random_adversarial
           to get a close initial boundary point."""
        _, C, H, W = shape
    ...
    def _low_freq_random(self, shape):
        """★ v13.1: full-dim torch.randn — used in main manifold_search_2d loop
           for high u diversity."""
    ...
    def find_random_adversarial(self, image, step=3.0, eps_max=15, n=60):
        # ★ v13.1: use DCT low-freq sampling to find close initial boundary
        num_calls = 0
        perturbed = image
        candidate = image
        max_calls=50
        for _ in range(n):
            u = self._low_freq_dct(image.shape).to(self.device)   # ★ DCT here
            u = u / torch.norm(u)
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v13.1-explore-mixed iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
