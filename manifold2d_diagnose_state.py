"""
Manifold2D state-variant 诊断脚本
---------------------------------
针对 pro_atk.py 的状态机变体做诊断。**重点观察状态切换**：
    iter  N  [EXPLOIT]  r_cur=...  Δr=...  inner=K/10  q+=Q  stag=N  Δrel=+X.XX%

关注三件事：
  1. EXPLORE / REFINE 状态**到底有没有触发**？多少次？什么时候？
  2. 在 exploit 变体的 11 轮长平台期（Image #1 iter 5-15），state 变体在这段是不是
     切到了 EXPLORE？切了之后有没有打破平台？
  3. R_REFINE_RATIO=0.15 在 CIFAR 上能不能触发？

用法:
    python manifold2d_diagnose_state.py                    # 默认 3 张图, iter=50
    python manifold2d_diagnose_state.py --num_img 1 --iter 30
"""
...
from pro_atk import Proposed_attack          # ← 注意：state 变体在 pro_atk.py
from utils import clip_image_values, valid_bounds


# ── 状态机参数（和 pro_atk.py 内部完全一致）───────────────────────────────────
...
class DiagnosticStateAttack(Proposed_attack):
    """Subclass of pro_atk.Proposed_attack with diagnostic instrumentation."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.last_actual_inner = 0
        # 统计状态切换次数
        self.state_transitions = []   # list of (it, from, to)

    # ────────────────────────────────────────────────────────────────────
    # 重写 manifold_search_2d：保持 return 签名不变，把 i 存到 self.last_actual_inner
    # 完全复制父类逻辑，只在 return 前记录
    # ────────────────────────────────────────────────────────────────────
    ...
    def Attack(self):
        ...
        # State tracking（和 pro_atk.py 内部一致）
        state  = STATE_EXPLOIT
        stag_k = 0
        dwell  = 0
        ...
        for it in range(outer_iter):
            ...
            # --- Compose u_new (same as original) ---
            ...
            # --- Compute post-search r for print ---
            r_after = float(torch.norm(x_adv - self.src_img).item())
            delta_r_this = r_now - r_after   # 这一轮实际压缩了多少 r

            # 状态切换标记
            tag = f'[{state:7s}]'
            if prev_state != state:
                tag = f'[{state:7s}*]'   # * = 本轮刚切换
            ...
            x_b = x_adv
            q_num = q_num + qs
            r_prev = r_now    # 注意：父类是在循环末尾 r_prev = r_now（一致）
...
def main():
    ...
    print(f'\n{"="*84}')
    print(f'  Manifold2D State-variant Diagnostic')
    print(f'  num_img = {args.num_img}   iteration = {args.iteration}')
    print(f'  model   = {args.model}    device = {device}')
    print(f'  Thresholds: EPS_PROGRESS={EPS_PROGRESS}  STAG_THRESH={STAG_THRESH}  '
          f'R_REFINE_RATIO={R_REFINE_RATIO}  MIN_DWELL={MIN_DWELL}')
    print(f'{"="*84}')
    ...
    for idx in range(len(cifar10)):
        ...
        attacker = DiagnosticStateAttack(
    ...
    for i, idx, gt, trans, final_r in all_transitions:
        if trans:
            ...
            for t_it, f, t in trans:
                print(f'      iter {t_it}: {f} → {t}')
        else:
            print(f'  Image #{i} (gt={gt}, final_r={final_r:.3f}): '
