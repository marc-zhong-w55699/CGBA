...
# ============================================================================
# ImageNet variant with LARGER init step size (option 3).
# Identical to M2D_explore_tuned_imagenet.py except:
#     find_random_adversarial:
#         step    : 3.0 → 5.0    (each walk step jumps further)
#         eps_max : 15  → 30     (max accumulated walk distance increases)
#
# Rationale:
#     ImageNet 初始 boundary norm 普遍 30~60，原 step=3 太保守，
#     单方向 5 步累计距离最多 45，常常打不穿。
#     step=5/eps_max=30 单方向最多 6 步，累计距离 5~105，更容易命中 adv。
#
# Expected impact (small):
#     ↓ init query  : 每张 ~146 → ~120 (省 ~20-30 q/image)
#     ↓ total query : 33 张省 ~660~1000 q
#     ↓ median norm : 6.95 → ~6.7~6.8 (边际改进)
#
# Algorithm (manifold_search_2d / outer loop) NOT changed.
# ============================================================================
...
class Proposed_attack():
    ...
    def find_random_adversarial(self, image, step=5.0, eps_max=30, n=60):    # ★ step=5.0, eps_max=30
        num_calls = 0
        perturbed = image
        candidate = image
        max_calls=50
