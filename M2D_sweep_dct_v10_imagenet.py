...
# ============================================================================
# ImageNet M2D-SWEEP (instrumented v10 random) — for boundary characterization
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 bump_max_per_image=100,           # ★ sweep: bump back on (default v10)
    ...
    def Attack(self):
        ...
        for it in range(outer_iter):
            ...
            if do_snapshot and len(self.snapshots) > 0:
                ...
                snap['theta_v10'] = float(best_angle)
                ...
                print(f'  [SWEEP it={it} '
                      ...
                      f'v10={math.degrees(best_angle):.2f}° '
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v10-sweep-dct iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
        ...
        if self.snapshots:
            ...
            for snap in self.snapshots:
                ...
                tm = max(lm, rm)
                v10 = snap['theta_v10'] or 0.0
                truth_angles.append(math.degrees(tm))
                v10_angles.append(math.degrees(v10))
                gaps.append(math.degrees(tm - v10))
            ta = np.array(truth_angles); va = np.array(v10_angles); ga = np.array(gaps)
            print(f'Sweep snapshots: {len(self.snapshots)}')
            print(f'  truth_θ  mean={ta.mean():.2f}°  P50={np.percentile(ta,50):.2f}°  max={ta.max():.2f}°')
            print(f'  v10 found mean={va.mean():.2f}°  P50={np.percentile(va,50):.2f}°  max={va.max():.2f}°')
