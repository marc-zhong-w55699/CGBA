"""
Surfree_attack_imaginenet_res50.py
用 SurFree 攻击 ImageNet 上的 ResNet-50。结构对齐
Non_targeted_attack_imaginenet_res50.py，输出 npz 字段一致
(norm, query, all_norms, all_queries, asr)，方便和 CGBA 结果横向合并。
"""
...
import time
from surfree_wrapper import SurFreeWrapper
...
max_queries      = 2000      # SurFree query budget per image
model_arc        = 'resnet50'
...
for attack_method in attack_methods:
    ...
    for image_iter1 in range(1, 51):
        ...
        if ground_label_int != int(orig_label):
            ...
            print('#################################################################################')
            print(f'Start: {attack_method} non-targeted will be run for {iteration} steps with max_queries={max_queries}')
            print('#################################################################################')
    ...
    if len(all_norms) > 0:
        ...
        if not os.path.exists('Surfree_results'):
            os.makedirs('Surfree_results')

        save_path = (f'Surfree_results/{attack_method}_nonTar_{model_arc}'
                     f'_dimReducFac_{dim_reduc_factor}'
                     f'_imgNum_{num_img}_iteration_{iteration}'
                     f'_maxQ_{max_queries}')
        np.savez(
            save_path,
