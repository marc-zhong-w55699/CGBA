...
class Proposed_attack():
    ...
    def Attack(self):
        ...
        # A 3-state machine adapts (lam1, lam2, lam3) to the current regime:
        #   EXPLOIT (default): balanced — trust evidence, keep noise margin
        #   EXPLORE          : stagnation → amplify noise to escape plateau
        #   REFINE           : close to x_o → trust momentum, cut noise
        ...
        EPS_PROGRESS   = 0.005   # per-iter relative-improvement threshold
        STAG_THRESH    = 5       # consecutive low-progress iters -> EXPLORE
        R_REFINE_RATIO = 0.15    # r_cur / r_init below this -> REFINE
        MIN_DWELL      = 2       # minimum iters to stay in a state
        ...
        for it in range(outer_iter):
            ...
            # --- State-machine update (zero query cost) ---
