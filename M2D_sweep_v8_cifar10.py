...
# ============================================================================
# SWEEP-INSTRUMENTED v8 for CIFAR-10 ViT
#
# Same v8 algorithm (walk + adaptive θ_max), but EVERY `sweep_every` outer
# iters, performs a FULL ANGULAR SWEEP on both sides (s=+1, s=-1) of the
# Thales circle to characterize the adv region.
#
# Sweep happens BEFORE the v8 walk runs on the same (x_o, x_b, u) state,
# so the snapshot represents the "ground truth" boundary that v8 walk
# samples from sparsely.
#
# Sweep cost (NOT counted against attack budget — for analysis only):
#   per snapshot: sweep_grid_size θ × 2 sides queries
#   per image:    (iter / sweep_every) snapshots × that
#
# Output: self.snapshots — list of dicts, one per snapshot.
#   Each dict contains: iter, norm, r, theta_v8 (what v8 walk chose),
#                       left_adv (bool[grid]), right_adv (bool[grid]).
# ----------------------------------------------------------------------------
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 sweep_theta_max_deg=60.0):    # largest θ to test
        self.model = model
    ...
    def _sweep_snapshot(self, x_o, x_b, u_raw, it, norm_val):
        ...
        self.snapshots.append({
            ...
            'left_adv':  left_adv,
            'right_adv': right_adv,
            'theta_v8':  None,    # filled after v8 walk runs
        })
    ...
    def _circ_inc_walk(self, x_o, r, v, u, s, theta_safety_cap,
                        init_angle, init_x):
        """★ v8 INCREMENT-DOUBLING WALK (clean baseline, no refine).
           ...
           No per-iter refine — empirically (50-img test) the +1q cost of
           BS refine outweighs the small precision gain. Precision pursuit
           moves to v9 line search (different DoF: r along d_new).
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v8-sweep iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
