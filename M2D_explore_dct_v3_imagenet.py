...
# ============================================================================
# ImageNet M2D-explore (λ=0.2,0.2,0.6) + DCT (square low-freq) + v3 geometry.
# Pure attack class.
#
# Combines:
#   - DCT direction (square low-freq k×k block, pure randn) — same as
#     M2D_explore_dct_imagenet.py
#   - v3 geometry: circular evolution + angle binary search
#   - Single inner step per outer iter (SurFree-style)
#
# Parameters:
#   dct_ratio   = 1/8             k = 28 for 224×224 (matches old DCT)
#   theta_max   = π/3 (60°)
#   BS_iter     = 7
#   iteration   = 700             target ~10000 queries
# ============================================================================
...
class Proposed_attack():
    ...
    def _low_freq_random(self, shape):
        """DCT low-freq random direction (square k×k block, pure randn)."""
        _, C, H, W = shape
        ...
        for it in range(outer_iter):
            ...
            if it % 20 == 0 or it == outer_iter - 1:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-explore-dct-v3 iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}')
