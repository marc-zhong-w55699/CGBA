...
#
# ★ v10 KEY CHANGE: decouple sign probe (LARGE) and walk (SMALL δ).
#
# Motivation:
#   Sweep experiment revealed:
#     - v8 picks wrong sign 43% of iters (probe 0.1° has 86% both-adv → blind pick +s)
#     - max(+s, -s) truth = 3.44° vs v8's 2.35° (+47%)
...
#     - walk g = 2 (unchanged)
#
# Cost: sign 2q (vs v8 1.07q) + walk same-ish = inner_q +0.5-0.7
# Expected per-q efficiency: +30-50%
#
# Motivation:
...
class Proposed_attack():
    ...
    def _circ_inc_walk(self, x_o, r, v, u, s, theta_safety_cap,
        ...
        θ_cur = init_angle
        # ★ v10: δ_init independent of init_angle (was tied to probe in v8)
    ...
    def manifold_search_2d(self, x_o, x_b,
                           ...
                           u=None,
                           theta_max_cur=None,
                           reject_at_floor=False,   # ★ v11: return best_angle=-1 if sign fails at floor
                           **kwargs):
        """Returns (x_e, num_calls, best_angle).

        ★ v11: if reject_at_floor=True and BOTH sides fail at sign_probe_angle,
        skip fallback halving and return (x_b, 2, -1.0). Signals outer loop
        to resample u and retry (u-rejection).
        """
        num_calls = 0
        ...
        # ★ v10: decoupled sign probe (larger) + walk (unchanged)
        ...
        # Test +s and -s at sign_probe_angle (always 2 q)
        ...
        if pos_adv and not neg_adv:
            ...
            # Both adv at sign_probe — ambiguous, pick +s arbitrarily
            # (a future v11 could probe at larger angle to differentiate)
            s = +1
            sign_found_angle = sign_probe_angle
            sign_found_x     = x_pos
        # else: both fail at sign_probe_angle → fallback halving

        # ★ v11: u-rejection — if both sides fail at sign_probe (boundary too
        # narrow in this u direction), signal outer loop to resample u.
        # -1.0 sentinel distinguishes reject from "sign truly found but zero".
        ...
        if sign_found_angle < self.sign_probe_floor:
            ...
            # ★ v11.2: walk δ_init = max(θ_max/8, sign_probe_angle)
            # Ensures first walk step at least as big as sign_probe (aggressive
            # outward expansion instead of tiny steps near probe angle).
    ...
    def Attack(self):
        ...
        for it in range(outer_iter):
            ...
            # ★ v11 u-rejection: try up to MAX_U_ATTEMPTS times to find a "good u"
            # (one whose boundary in this direction is ≥ sign_probe_floor).
            # If all attempts fail, do the last try with reject_at_floor=False
            # (fall through to fallback halving as in v10).
            qs_total = 0
            for u_attempt in range(self.max_u_attempts):
                ...
                qs_total += qs
                if best_angle >= 0:
                    break   # good u (or last attempt, whatever it returned)
            qs = qs_total
