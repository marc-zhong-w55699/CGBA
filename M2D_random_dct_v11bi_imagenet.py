...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 sign_probe_floor=math.pi/180,     # ★ v11: 1° (was 0.8°, v11 rejection catches fail)
                 # ★ v11 u-rejection + halving cap
                 max_u_attempts=4,                 # retry u up to N times
                 halving_min=math.pi/720,          # 0.25° → halving only 1 test
                 # ★ v11-bestinit: collect K adv candidates for init
                 keep_best_init=3):                # ~65q cost, expect -50% init r
        self.model = model
        ...
        # ★ v11-bestinit
        self.keep_best_init = keep_best_init
    ...
    def find_random_adversarial(self, image, step=3.0, eps_max=15, n=60):
        """★ v11-bestinit: collect self.keep_best_init adv candidates, return min-r one.
           Cost: extra ~50-100q per image, expected -30-50% init r.
        """
        num_calls = 0
        max_calls = 50
        candidates = []   # list of (r, x_b) tuples
        ...
        for _ in range(n):
            ...
            if is_adv == 1:
                x_b, bin_calls = self.bin_search(image, candidate, max_calls)
                num_calls += bin_calls
                r = float(torch.norm(x_b - image).item())
                candidates.append((r, x_b))
                if len(candidates) >= self.keep_best_init:
                    break   # ★ 收集够 K 个了

        if candidates:
            # ★ 返回 r 最小的 candidate
            best = min(candidates, key=lambda x: x[0])
            return best[1], num_calls
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v11bi-k3 iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
