...
# ============================================================================
# ImageNet M2D-state (state machine: EXPLOIT/EXPLORE/REFINE)
...
#
# v5b key insight (same as random/explore/exploit/refine dct_v5):
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-state-dct-v5 iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
                          f'   state={state}' +
