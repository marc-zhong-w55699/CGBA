...
# ============================================================================
# ImageNet M2D-explore (λ=0.2,0.2,0.6) + DCT-SF direction + v3 geometry.
# Pure attack class.
#
# Combines:
#   - DCT-SF direction (zigzag mask, image-conditioned, ternary × tanh)
#   - v3 geometry: circular evolution + angle binary search
#   - Single inner step per outer iter (SurFree-style)
#
# Parameters:
#   freq_range  = (0, 0.5)        SurFree default
#   tanh_gamma  = 1.0             SurFree default
#   theta_max   = π/3 (60°)       circular search range
#   BS_iter     = 7               binary search depth
#   iteration   = 700             target ~10000 queries (q/iter ≈ 14 on ImageNet)
# ============================================================================
        ...
        for it in range(outer_iter):
            ...
            if it % 20 == 0 or it == outer_iter - 1:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v3-explore-dctsf iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}')
