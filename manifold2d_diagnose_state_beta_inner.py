"""
Manifold2D state + β + inner_n 联合 ablation
---------------------------------------------
**改两个参数**：β=π/60, inner_n=15。tol 保持原版 1e-4。
...
目的：验证 inner_n=15 是否独立贡献了"state+tuned 比 state+β only 好"的差距。

预期对照（重点看 Image #2）：
  - state 原版:           9.34
  - state + β only:       13.27（差 +3.93）
  - state + β + inner_n: ??（如果接近 11.26，则 inner_n 提供大部分补偿；
                               如果接近 13.27，则 tol 才是补偿来源）
  - state + tuned (3个):  11.26

用法:
    python manifold2d_diagnose_state_beta_inner.py --num_img 3 --iter 100
"""
...
# ── ★ 改两个参数 ──────────────────────────────────────────────────────────────
TUNED_BETA    = math.pi / 60   # ★
TUNED_INNER_N = 15             # ★
ORIG_TOL      = 1e-4           # 保持原版


class DiagnosticStateBetaInnerAttack(Proposed_attack):
    """State + β=π/60 + inner_n=15; tol unchanged."""
...
def main():
    ...
    print(f'\n{"="*90}')
    print(f'  Manifold2D State + ★ β + inner_n (tol unchanged)')
    print(f'  num_img = {args.num_img}   iteration = {args.iteration}')
    print(f'  ★ Changed: beta=π/{int(math.pi/TUNED_BETA)}  inner_n={TUNED_INNER_N}')
    print(f'  UNCHANGED: tol={ORIG_TOL}  (state thresholds all original)')
    ...
    for idx in range(len(cifar10)):
        ...
        attacker = DiagnosticStateBetaInnerAttack(
