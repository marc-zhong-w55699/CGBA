...
# DCT:      keep top-(H·dct_ratio) × (W·dct_ratio) low-freq coefficients,
#           sample random N(0,1) there, iDCT to spatial direction.
# ============================================================================
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 dct_ratio=1.0/8,
                 theta_max=math.pi / 3,
                 theta_min_bound=math.pi / 60,    # ★ v5: lower bound 3°
                 theta_max_bound=math.pi / 2,     # ★ v5: upper bound 90°
                 grow_factor=1.15,                # ★ v5b: expand multiplier
                 shrink_factor=0.85,              # ★ v5: shrink multiplier
                 shrink_thresh=0.15,              # ★ v5b: shrink only if best_θ < 15% × θ_max
    ...
    def Attack(self):
        ...
        for it in range(outer_iter):
            ...
            if best_angle > 0:
                ...
                # else: hold
            # ★ v5b: removed sign-fail shrink (was too aggressive; sign-fail often transient)
            ...
            if it % 50 == 0 or it == outer_iter - 1:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-refine-dct-v5 iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
