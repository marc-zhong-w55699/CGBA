...
# ============================================================================
# CIFAR-10 M2D-SWEEP (instrumented v11.2 random) — boundary characterization
#   + v3 geometry (circular evolution)
#   + v5b adaptive theta_max
#   + v6.4 reverse floor-bump (CIFAR-tuned; enabled)
#   + v8 increment-doubling walk (no halving in walk)
#   + v10 decoupled sign probe (cap 8°, floor 1°)
#   + v11 sign_probe_floor = 1° (matches attack v11.2)
#   + v11.1 skip walk when sign_found_angle < probe_floor
#   + v11.2 δ_init = max(θ_max/8, sign_probe_angle) — walk step ≥ probe
#
# Sweep instrumentation:
#   - Every `sweep_every` iters, sample 60 log-spaced θ ∈ [0.1°, 90°]
#   - Test BOTH s=+1 and s=-1 at each θ (120 total tests per snapshot)
#   - Snapshot records left_adv/right_adv masks + v11.2's chosen θ
#   - Sweep queries go to self.all_queries but NOT counted in q_num.
#
# Purpose: characterize CIFAR truth distribution under v11.2 params
# (init 50°, cap 60°, probe floor 1°, δ = max(θ_max/8, probe)).
# ----------------------------------------------------------------------------
# Original v8 design retained:
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 # ★ v10 decoupled sign probe (cap + v11 floor)
                 sign_probe_cap=math.pi/22.5,  # max sign_probe (= 8°)
                 sign_probe_floor=math.pi/180, # ★ v11: 1° (matches CIFAR v11.2)
                 # ★ v11.1 walk halving stop
                 halving_min=math.pi/720,      # ★ v11.1: 0.25° (CIFAR-tuned)
                 # ★ sweep params
                 sweep_every=20,               # snapshot interval (outer iters)
                 sweep_grid_size=60,           # number of θ samples per sweep
                 sweep_theta_min_deg=0.1,      # smallest θ to test
                 sweep_theta_max_deg=90.0):    # ★ extended: 60 → 90° (see ImageNet sweep result)
        self.model = model
        ...
        self.sweep_grid_size = sweep_grid_size
        # log-spaced grid in radians, sweep_theta_min_deg → sweep_theta_max_deg
        self.theta_grid = np.exp(np.linspace(
            np.log(np.radians(sweep_theta_min_deg)),
            np.log(np.radians(sweep_theta_max_deg)),
            sweep_grid_size,
        ))
        self.snapshots = []   # filled during Attack()
    ...
    def _sweep_snapshot(self, x_o, x_b, u_raw, it, norm_val):
        """Sweep θ_grid on both s=+1 and s=-1, record adv masks.

           Called BEFORE v8 walk runs, so it captures the state v8
           is about to sample sparsely. Sweep queries are recorded
           in self.all_queries (via is_adversarial) but NOT added to
           the attack's q_num counter — they're for analysis only.
        """
        r = torch.norm(x_b - x_o)
        v = (x_b - x_o) / r

        # Project & normalize u (same as inside manifold_search_2d)
        u = u_raw.to(self.device)
        u = u - torch.dot(u.reshape(-1), v.reshape(-1)) * v
        u_norm = torch.norm(u)
        if u_norm < 1e-8:
            return    # degenerate u, skip snapshot
        u = u / u_norm

        n_grid = len(self.theta_grid)
        left_adv  = np.zeros(n_grid, dtype=bool)
        right_adv = np.zeros(n_grid, dtype=bool)

        for i, theta in enumerate(self.theta_grid):
            x_pos = self._circ_x_at(x_o, r, v, u, +1, float(theta))
            right_adv[i] = (self.is_adversarial(x_pos) == 1)
            x_neg = self._circ_x_at(x_o, r, v, u, -1, float(theta))
            left_adv[i]  = (self.is_adversarial(x_neg) == 1)
        ...
        self.snapshots.append({
            ...
            'left_adv':  left_adv,
            'right_adv': right_adv,
            'theta_v11':  None,    # filled after v8 walk runs
        })
    ...
    def _circ_inc_walk(self, x_o, r, v, u, s, theta_safety_cap,
        ...
        """★ v8 increment-doubling walk (no halving-after-fail in v11.2).
    ...
    def manifold_search_2d(self, x_o, x_b,
        ...
        # ★ v10: decoupled sign probe (test BOTH sides at min(max(θ_max/4, floor), cap))
        ...
        if s == 0:
            ...
            # ★ v11.1: use self.halving_min instead of beta_min
        ...
        # δ_init = max(θ_max/8, sign_probe_angle) — walk step ≥ probe
    ...
    def Attack(self):
        ...
        for it in range(outer_iter):
            ...
            # ★ sweep snapshot BEFORE v8 walk (same (x_o, x_b, u_new) state)
            do_snapshot = (it % self.sweep_every == 0)
            if do_snapshot:
                # use the most recent norm; for iter 0 use norm_initial
                cur_norm = float(norms[-1]) if isinstance(norms[-1], float) else float(norms[-1].item())
                self._sweep_snapshot(self.src_img, x_b, u_new, it, cur_norm)
            ...
            if do_snapshot and len(self.snapshots) > 0:
                ...
                snap['theta_v11'] = float(best_angle)
                ...
                print(f'  [SWEEP it={it} '
                      ...
                      f'v11={math.degrees(best_angle):.2f}° '
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v11.2-sweep iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
        ...
        if self.snapshots:
            ...
            print(f'  v11.2 found mean={va.mean():.2f}°  P50={np.percentile(va,50):.2f}°  max={va.max():.2f}°')
            print(f'  gap        mean={ga.mean():.2f}°  P50={np.percentile(ga,50):.2f}°  max={ga.max():.2f}°  (truth - v11.2)')
        print(f'────────────────────────────────────────────────')
