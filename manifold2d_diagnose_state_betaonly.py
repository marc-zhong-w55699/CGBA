"""
Manifold2D state + 仅改 beta 的 ablation 脚本
---------------------------------------------
**只改一个参数**：beta = π/30 → π/60。其它全部和 pro_atk.py 原版一致。

    ┌─────────────┬─────────────┬─────────────┐
    │ 参数        │ pro_atk 原版│ 本脚本      │
    ├─────────────┼─────────────┼─────────────┤
    │ beta        │ π/30 ≈ 6°   │ π/60 ≈ 3°   │  ★ 只动这一个
    │ inner_n     │ 10          │ 10 (保持)   │
    │ tol         │ 1e-4        │ 1e-4 (保持) │
    └─────────────┴─────────────┴─────────────┘

状态机阈值全部保持原 pro_atk.py 值。

目的：精确定位"state+tuned 在 Image #2 上变差到 11.26"是否由 beta 引起。

预期对照：
  - state 原版 Image #2: r=9.34   （beta=π/30，"漂亮"救场）
  - state+tuned Image #2: r=11.26  （三个 tuned 一起改，变差了）
  - 本脚本（只 beta=π/60）：??     （如果接近 11，说明 beta 是元凶）

用法:
    python manifold2d_diagnose_state_betaonly.py --num_img 3 --iter 100
"""
...
class DiagnosticStateBetaOnlyAttack(Proposed_attack):
    """State machine + only β tuned. inner_n=10 and tol=1e-4 are the original."""

    def __init__(self, *args, **kwargs):
        # ★ 保持原版 tol
        kwargs.setdefault('tol', ORIG_TOL)
        super().__init__(*args, **kwargs)
        self.last_actual_inner = 0
        self.state_transitions = []

    # ────────────────────────────────────────────────────────────────────────
    # 复制父类 manifold_search_2d；唯一改动：beta 默认 π/60；记录 actual_inner
    # ────────────────────────────────────────────────────────────────────────
    def manifold_search_2d(self, x_o, x_b,
                           alpha=0.99,
                           beta=TUNED_BETA,             # ★ 唯一改动
    ...
    # ────────────────────────────────────────────────────────────────────────
    # Attack：保留 pro_atk 状态机；inner_n 保持原版 10；显式传调过的 beta
    # ────────────────────────────────────────────────────────────────────────
    def Attack(self):
        norms = []
        n_query = []
...
def main():
    ...
    print(f'\n{"="*90}')
    print(f'  Manifold2D State + ★ ONLY β tuned (ablation)')
    print(f'  num_img = {args.num_img}   iteration = {args.iteration}')
    print(f'  ★ Changed: beta=π/{int(math.pi/TUNED_BETA)}')
    print(f'  UNCHANGED: inner_n={ORIG_INNER_N}  tol={ORIG_TOL}  '
          f'(state thresholds all original)')
    ...
    for idx in range(len(cifar10)):
        ...
        attacker = DiagnosticStateBetaOnlyAttack(
