...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 verbose_control='Yes',
                 dct_ratio=1.0/8,
                 theta_max=math.pi * 7 / 18,      # ★ Test 1: 70° init (was 50°)
                 theta_min_bound=math.pi / 90,    # 2°
                 theta_max_bound=math.pi * 7 / 18,# ★ Test 1: 70° cap (matches init, was 60°)
                 grow_factor=1.15,
                 shrink_factor=0.85,
                 shrink_thresh=0.15,
                 BS_iter=3,
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v11.2-test1-init70 iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
