...
# ============================================================================
# ImageNet M2D-SWEEP (instrumented v11.2 random) — boundary characterization
#   + DCT (square low-freq block, dct_ratio=1/8)
#   + v3 geometry (circular evolution)
#   + v5b adaptive theta_max
#   + v6.4 reverse floor-bump (disabled by default in v11)
#   + v8 increment-doubling walk with halving
#   + v10 decoupled sign probe (cap 8°, floor 1°)
#   + v11 sign_probe_floor = 1° (was 0.8°)
...
#   - Every `sweep_every` iters, sample 60 log-spaced θ ∈ [0.1°, 60°]
...
#
# Purpose: characterize true truth distribution under v11.2 params
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 verbose_control='Yes',
                 dct_ratio=1.0/4,   # ★ dct14 sweep: match CGBA/TtBA convention (s=4)
                 theta_max=math.pi / 3.6,         # ★ v11.2: 50° init (matches attack v11.2)
                 ...
                 shrink_thresh=0.15,
                 BS_iter=3,
                 # ★ v6.4 reverse bump params (ImageNet-tuned)
                 bump_best_theta_thresh=math.pi / 180,  # 1° (ImageNet best_θ P50 ≈ 1°)
                 bump_ratio_thresh=5.0,
                 bump_streak=2,
                 bump_target=math.pi / 360,        # 0.5°
                 bump_cooldown=50,
                 bump_warmup=100,
                 bump_max_per_image=100,           # ★ sweep: bump on (observe under bump)
                 bump_norm_gate=0.0,
                 # ★ v10 decoupled sign probe (cap + v11 floor)
                 sign_probe_cap=math.pi/22.5,      # max sign_probe (= 8°)
                 sign_probe_floor=math.pi/180,     # ★ v11: 1° (was v10's 0.8°)
                 # ★ v11.1 walk halving
                 halving_min=math.pi/360,          # 0.5° halving stop
                 # ★ sweep params (instrumentation, NOT counted in attack q_num)
    ...
    def _sweep_snapshot(self, x_o, x_b, u_raw, it, norm_val):
        """Full angular sweep on both s=+1, s=-1 at theta_grid. Snapshot records
           ground-truth adv mask before v10 walk runs on the same (x_o, x_b, u)
           state. Sweep queries go to self.all_queries but NOT q_num."""
    ...
    def Attack(self):
        ...
        for it in range(outer_iter):
            ...
            # ★ sweep snapshot BEFORE v10 walk (same (x_o, x_b, u_new) state)
            do_snapshot = (it % self.sweep_every == 0)
            if do_snapshot:
                cur_norm = float(norms[-1].item()) if hasattr(norms[-1], 'item') else float(norms[-1])
            ...
            # ★ record v10's chosen θ + print sweep summary line
            if do_snapshot and len(self.snapshots) > 0:
                snap = self.snapshots[-1]
                snap['theta_v11'] = float(best_angle)  # v11.2's chosen θ
                left_max  = float(self.theta_grid[snap['left_adv']].max())  if snap['left_adv'].any()  else 0.0
                right_max = float(self.theta_grid[snap['right_adv']].max()) if snap['right_adv'].any() else 0.0
                truth_max = max(left_max, right_max)
                n_left  = int(snap['left_adv'].sum())
                n_right = int(snap['right_adv'].sum())
                print(f'  [SWEEP it={it} '
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v11.2-dct14-sweep iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
        ...
        if self.snapshots:
            ...
            print(f'  gap      mean={ga.mean():.2f}°  P50={np.percentile(ga,50):.2f}°  max={ga.max():.2f}°')
        print(f'────────────────────────────────────────────────')
