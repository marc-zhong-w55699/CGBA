...
# ============================================================================
# ImageNet M2D-explore (λ=0.2,0.2,0.6)  v12 = v11.2 + walk halving
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
                 ...
                 # ★ v12 walk-halving
                 walk_halving_min=math.pi/720):    # ★ v12: 0.25° walk halving stop
        self.model = model
    ...
    def _circ_inc_walk(self, x_o, r, v, u, s, theta_safety_cap,
        ...
        while True:
            ...
            if self.is_adversarial(x_test) == 1:
                ...
                # ★ v12: halving retry after fail
                found_in_halving = False
                while δ > self.walk_halving_min:
                    ...
                    if self.is_adversarial(x_h) == 1:
                        ...
                        x_best     = x_h
                        θ_cur      = θ_h
                        δ         *= 2.0    # continue outer walk with doubled δ
                        found_in_halving = True
                        break
                if not found_in_halving:
                    break    # halving exhausted or δ too small → truly at boundary
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v12-explore-walkhalve iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
