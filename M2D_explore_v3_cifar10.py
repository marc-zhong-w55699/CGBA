...
# ============================================================================
# CIFAR-10 M2D-explore + SurFree-style A+B (circular evolution + binary search).
# Pure attack class. No DCT — plain random d3 + d1/d2 momentum.
#
# vs M2D_random_v3_cifar10.py:
#     λ : 0, 0, 1 (random) → 0.2, 0.2, 0.6 (explore)
#     保留 d1 progress + d2 momentum，让 u 不只依赖 random noise
#
# v3 几何（不变）:
#     x(θ) = X + r·cos(θ)·(v·cos(θ) + s·u·sin(θ))
#     binary search θ in [0, theta_max=π/3]
#     inner_n = 1 (SurFree-style)
#     iteration = 1000 (target ~10000 q)
# ============================================================================
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v3-explore iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}')
