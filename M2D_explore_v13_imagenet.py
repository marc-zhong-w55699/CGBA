...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, dim_reduc_factor=4,
        ...
        self._H, self._W = H, W
        # ★ v13: NO DCT — u sampled from full-dim Gaussian noise (like CIFAR)



    def _low_freq_random(self, shape):
        """★ v13: keep method name for API compatibility, but drop DCT projection.
           Returns full-dim torch.randn — high-dim search, high u diversity."""
        ...
        for it in range(outer_iter):
            ...
            if it % 50 == 0 or it == outer_iter - 1 or bump_log:
                if self.verbose_control == 'Yes':
                    print('Manifold2D-v13-explore-nodct-halve iter -> ' + str(it) +
                          '   Queries ' + str(q_num) +
                          '   norm -> ' + f'{norm.item():.3f}' +
                          f'   inner_q={qs}' +
